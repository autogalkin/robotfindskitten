...
int main(int argc, char *argv[]) {
  try {
    std::unique_ptr<PROCESS_INFORMATION, void (*)(const PROCESS_INFORMATION *)>
    ...
    std::cout << "Launch notepad.exe..." << std::endl;
    {
      ...
      if (!CreateProcess(cmd.data() // lpApplicationName
                         ...
                         nullptr, &startup_info, proc_info.get())) {
        error("Launch notepad.exe process..");
      }
    }

    std::cout << "`Notepad process id is " << GetProcessId(proc_info->hProcess)
              << std::endl;
    ...
    const auto dll_path = std::filesystem::absolute("notepadgame.dll").string();
