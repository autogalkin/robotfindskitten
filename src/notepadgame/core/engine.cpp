...
{
    edit_window_ = CreateWindowExW(dwExStyle,
...
{
    direct_function_ = reinterpret_cast<npi_t (__cdecl *)(sptr_t, int, uptr_t, sptr_t)>(GetProcAddress(native_dll_.get(), "Scintilla_DirectFunction"));
    //direct_function_ = reinterpret_cast<npi_t (__cdecl *)(sptr_t, int, uptr_t, sptr_t)>(SendMessage(get_native_window(),SCI_GETDIRECTFUNCTION, 0, 0));
