...
BOOL APIENTRY DllMain(const HMODULE h_module, const DWORD ul_reason_for_call,
  ...
  if (ul_reason_for_call == DLL_PROCESS_ATTACH) { /**/
    ...
    const auto np_module =
        GetModuleHandleW(nullptr); // get the module handle of the notepad.exe

    [[maybe_unused]] constexpr uint8_t start_options =
        notepader::options::empty
#ifdef NDEBUG
        | notepader::hide_selection | notepader::kill_focus |
        notepader::disable_mouse
#endif // NDEBUG
        | notepader::show_eol | notepader::show_spaces;

    np.get_on_open().connect([] { game::start(); });
