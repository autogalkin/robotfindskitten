...
struct vec_t : public Eigen::Vector2<SizeType> {
  ...
  static vec_t<SizeType> null() {
    return vec_t<SizeType>{static_cast<SizeType>(0), static_cast<SizeType>(0)};
  }
};
...
inline position from_location(const location &location) {
  return {std::lround(location.line()), std::lround(location.index_in_line())};
}
} // namespace position_converter
...
struct boundbox {
  ...
  [[nodiscard]] bool contains(const boundbox &b) const {
    return pivot.line() <= b.pivot.line() && end().line() <= b.end().line() &&
  ...
  [[nodiscard]] bool intersects(const boundbox &b) const {
    return !(pivot.line() >= b.end().line() || end().line() <= b.pivot.line() ||
...
struct sprite {
  ...
  using data_type =
      Eigen::Matrix<char_size, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>;
