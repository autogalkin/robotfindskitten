...
void engine::init_direct_access() {
  ...
  // direct_function_ = reinterpret_cast<npi_t (__cdecl *)(sptr_t, int, uptr_t,
  // sptr_t)>(SendMessage(get_native_window(),SCI_GETDIRECTFUNCTION, 0, 0));
  direct_wnd_ptr_ =
      SendMessage(get_native_window(), SCI_GETDIRECTPOINTER, 0, 0);
  // thread_safe_function_ =  reinterpret_cast<int64_t (__cdecl *)(sptr_t, int,
  // uptr_t, sptr_t)>(&SendMessageW); //
