...
LRESULT notepader::hook_wnd_proc(HWND hwnd, const UINT msg, const WPARAM wp,
                                 const LPARAM lp) {
  ...
  {
    {
      ...
      if (const HWND stbar =
              FindWindowExW(notepader::get().get_main_window(), nullptr,
      ...
      PostMessage(notepader::get().get_main_window(), WM_SIZE, SIZE_RESTORED,
                  MAKELPARAM(rc.right - rc.left, rc.bottom - rc.top));
    }

    get().input_ = std::make_unique<input>();
    get().get_on_open().connect([] { gamelog::cout("Connected"); });
    get().on_open_();
    // disable
  ...
  case WM_SIZE: {
    ...
    if (get().get_engine())
      get().get_engine()->get_on_resize()(width, height);
    break;
  }
  default:
    break;
  }
  return CallWindowProc(reinterpret_cast<WNDPROC>(get().original_proc_), hwnd,
                        msg, wp, lp);
}
      ...
      [](const LPMSG lpMsg, const HWND hWnd, const UINT wMsgFilterMin,
        ...
        while (PeekMessageW(lpMsg, hWnd, wMsgFilterMin, wMsgFilterMax,
          ...
          case WM_KEYUP: {
            ...
            if (lpMsg->wParam != VK_BACK && lpMsg->wParam != VK_DELETE) {
              if (auto &input = get().get_input_manager(); input)
                input->receive(lpMsg);
              // Block
  ...
  return iat_hook::hook_import<decltype(original)>(
      ...
      [](DWORD dwExStyle, LPCWSTR lpClassName, LPCWSTR lpWindowName,
                                 ...
                                 // custom window
        {
          ...
          out_hwnd = get().engine_->create_native_window(
        ...
        if (!lstrcmpW(lpClassName, L"Notepad")) {
          ...
          // patch the wnd proc
          get().original_proc_ =
              GetWindowLongPtr(get().main_window_, GWLP_WNDPROC);
          SetWindowLongPtr(get().main_window_, GWLP_WNDPROC,
