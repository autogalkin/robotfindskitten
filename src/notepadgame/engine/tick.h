...
public:
  ...
  virtual void tickframe() {
    ...
    /*250ms is the limit put in place on the frame time to cope with the spiral
     *of death. It doesn't have to be 250ms exactly but it should be
    ...
    seconds_tstep =
        time_point_cast<std::chrono::seconds>(std::chrono::steady_clock::now());
    ++frame_count;
    if (seconds_tstep != pt) {
