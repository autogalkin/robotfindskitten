...
class backbuffer {
  public:
    ...
    [[nodiscard]] position
    global_position_to_buffer_position(const position& position) const noexcept;
    [[nodiscard]] bool
    is_in_buffer(const position& global_position) const noexcept;
  ...
  private:
    ...
    void traverse_sprite_positions(
        ...
        const std::function<void(const position&, char_size part_of_sprite)>&
            visitor) const;
  ...
  public:
    ...
    template <typename T, typename... Args>
        requires std::derived_from<T, ecs_processor> &&
                 std::is_constructible_v<T, world*, Args...>
    void push(Args&&... args) {
        data_.emplace_back(
            std::make_unique<T>(w_, std::forward<Args>(args)...));
    }
...
class world final
    : public backbuffer,
      public tickable // NOLINT(cppcoreguidelines-special-member-functions)
{
  ...
  public:
    ...
    void
    spawn_actor(const std::function<void(entt::registry&, const entt::entity)>&
                    for_add_components) {
