    ...
    if (const quadnode &node = nodes_[nd.i]; node.isleaf()) {
      ...
      if (auto quadrant = get_quadrant(nd.rect, start_rect);
        ...
        to_process.push(quad_node_data{
            ...
            node.first_child + static_cast<index>(quadrant), nd.depth + 1});
      }
    }
  }
}
...
boundbox collision::quad_tree::compute_child_box(const boundbox &parent,
  ...
  case quadrant::bottom_left:
    return {
        {parent.pivot.line() + child_size.line(), parent.pivot.index_in_line()},
        child_size};
...
void collision::query::execute(entt::registry &reg, gametime::duration delta) {
  ...
  for (const auto view =
    ...
    view.get<collision::agent>(entity).index_in_quad_tree = tree.insert(
  ...
  { // query
    const auto view = reg.view<location_buffer, const shape::sprite_animation,
    ...
    for (const auto entity : view) {
      ...
      tree.query(view.get<shape::sprite_animation>(entity)
                         .current_sprite()
                         .bound_box() +
                     (position_converter::from_location(current_location +
                                                        translation.get())),
      ...
      if (!collides.empty()) {
        ...
        for (const auto collide : collides) {
          if (const auto cl = tree.get_entity(collide).id; cl != entity) {
            const auto cl_resp = view.get<on_collide>(cl).call(reg, cl, entity);
            if (const auto resp =
                    view.get<on_collide>(entity).call(reg, entity, cl);
    ...
    for (const auto entity : view) {
      if (!view.get<location_buffer>(entity).translation.get().is_null() ||
