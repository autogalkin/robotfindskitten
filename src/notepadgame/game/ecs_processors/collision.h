...
private:
  struct quadnode {
    ...
    [[nodiscard]] bool isleaf() const { return count_entities != is_branch; }
  };
  ...
  // for each cell it occupies, an "element node" is inserted which indexes that
  // element. singly-linked index list node into an array
  ...
  // free list is empty, at which point we simply insert 4 nodes to the back of
  // the nodes array.
