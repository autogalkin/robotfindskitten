  ...
  void execute(entt::registry &reg, gametime::duration delta) override {
    for (const auto view =
             reg.view<location_buffer, shape::sprite_animation,
                      shape::on_change_direction, shape::render_direction>();
      ...
      if (auto &[dir] = view.get<shape::render_direction>(entity);
          ...
          static_cast<int>(std::copysign(
              1, translation.get().index_in_line())) != static_cast<int>(dir)) {
  ...
  void execute(entt::registry &reg, gametime::duration delta) override {
    ...
    for (const auto view =
      ...
      if (translation.is_dirty()) {
        ...
        get_world()->draw(position_converter::from_location(current),
