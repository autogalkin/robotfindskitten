...
public:
    explicit screen_change_handler(world* w, engine* e): ecs_processor(w){
        size_changed_connection = e->get_on_resize().connect([this](const uint32_t width, const uint32_t height){
            on_resize(get_world()->get_registry(), width, height);
        });
        scroll_changed_connection_ = e->get_on_scroll_changed().connect([this](const position& new_scroll){
    ...
    void on_resize(entt::registry& reg, const uint32_t width, const uint32_t height) {
