...
struct actor {
  ...
  static void make_base_renderable(entt::registry &reg, const entt::entity e,
    ...
    reg.emplace<location_buffer>(e, std::move(start),
...
struct projectile final {
  ...
  static void
  make(entt::registry &reg, const entt::entity e, location start, velocity dir,
       const std::chrono::milliseconds life_time = std::chrono::seconds{1}) {
    reg.emplace<projectile>(e);
    reg.emplace<shape::sprite_animation>(
        e, std::vector<shape::sprite>{{shape::sprite::from_data{U"-", 1, 1}}},
        static_cast<uint8_t>(0));
    reg.emplace<visible_in_game>(e);
    ...
    reg.emplace<timeline::what_do>(
        e, [duration = std::chrono::duration<double>(life_time).count(),
          ...
          if (std::lround(i * 10) != 0 && call) {
            reg_.get<shape::sprite_animation>(e_).current_sprite().data(0, 0) =
                '_';
            call = false;
          }
        });
            ...
            auto create_fragment = [&r_, &lb](location offset, velocity dir_) {
              ...
              r_.emplace<location_buffer>(
                  ...
                  location{lb.current.line() + offset.line(),
                           lb.current.index_in_line() + offset.index_in_line()},
                  df::dirtyflag<location>{{}, df::state::dirty});
...
struct character final {
  ...
  static void make(entt::registry &reg, const entt::entity e, location l) {
    ...
    reg.emplace<visible_in_game>(e);
    reg.emplace<location_buffer>(e, std::move(l),
                                 df::dirtyflag<location>{{}, df::state::dirty});
  ...
  static void add_top_down_camera(entt::registry &reg, const entt::entity e) {
    ...
    reg.emplace<timeline::what_do>(e, [](entt::registry &reg_,
                                         const entt::entity owner, direction) {
      ...
      const npi_t width = notepader::get().get_engine()->get_window_width() /
      ...
      if (pos.index_in_line() > width + scroll.index_in_line() ||
          pos.index_in_line() < scroll.index_in_line()) {
        c_move_pos = static_cast<int>(pos.index_in_line() -
                                      scroll.index_in_line() - width / 2);
      }
      int l_move_pos = 0;
      ...
      if (l_move_pos != 0 || c_move_pos != 0) {
        ...
        reg_.emplace<timeline::what_do>(
            scroll_timeline,
            [lifetime = 0, t = 0, i = 0, j = 0, c_move_pos,
             c_sign = static_cast<int>(std::copysign(1, c_move_pos)),
             l_move_pos,
             l_sign = static_cast<int>(std::copysign(1, l_move_pos))](
                entt::registry &registry, const entt::entity scr_timeline,
                direction) mutable {
              if (lifetime >
                  std::max(std::abs(c_move_pos), std::abs(l_move_pos))) {
        ...
        reg_.emplace<life::death_last_will>(
            scroll_timeline, [owner](entt::registry &r_, const entt::entity) {
  ...
  static void process_movement_input(entt::registry &reg, const entt::entity e,
                                     const input::key_state_type &state) {
