...

  {
    ...
    auto spawn_block = [&i, &j](entt::registry &reg,
      ...
      actor::make_base_renderable(
          reg, entity, {6 + static_cast<double>(i), 4 + static_cast<double>(j)},
          1, {shape::sprite::from_data{U"█", 1, 1}});
    ...
    w->spawn_actor([](entt::registry &reg, const entt::entity entity) {
      ...
      reg.emplace<screen_resize>(
          entity, [&reg, entity](const uint32_t width, const uint32_t height) {
      ...
      reg.emplace<screen_scroll>(
          entity, [&reg, entity](const position &new_scroll) {
            auto &[current, translation] = reg.get<location_buffer>(entity);
            translation.pin() =
                location{0., static_cast<double>(new_scroll.index_in_line()) -
