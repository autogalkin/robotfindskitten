...
inline double easeinrange(const double value,
                          const std::pair<double, double> range,
                          const double duration, double (*ease_func)(double)) {
...
inline double easeInSine(const double t) { return std::sin(pi / 2 * t); }

inline double easeOutSine(double t) { return 1 + std::sin(pi / 2 * (--t)); }

inline double easeInOutSine(const double t) {
    return 0.5 * (1 + std::sin(pi * (t - 0.5)));
}

constexpr double easeInQuad(const double t) { return t * t; }

constexpr double easeOutQuad(const double t) { return t * (2 - t); }

constexpr double easeInOutQuad(const double t) {
    return t < 0.5 ? 2 * t * t : t * (4 - 2 * t) - 1;
}

constexpr double easeInCubic(double t) { return t * t * t; }

constexpr double easeOutCubic(double t) { return 1 + (--t) * t * t; }

constexpr double easeInOutCubic(double t) {
    return t < 0.5 ? 4 * t * t * t : 1 + (--t) * (2 * (--t)) * (2 * t);
}

constexpr double easeInQuart(const double t) { return t * t * t * t; }
...
inline double easeInExpo(double t) { return (std::pow(2, 8 * t) - 1) / 255; }

inline double easeOutExpo(double t) { return 1 - std::pow(2, -8 * t); }
...
inline double easeInCirc(double t) { return 1 - std::sqrt(1 - t); }

inline double easeOutCirc(double t) { return std::sqrt(t); }
